...
for train_image in train_images:
    ...
    im_gray = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
     #keyPnts & descriptor of trained images
 ...
 # Create the identity filter,usefull for sharpness
    ...
    if ret == True:
        ...
        for cnt in contours:
            ...
            for index,des in enumerate(X_test):
                 try:
                     matches = flann.knnMatch(np.array(des,dtype=np.float32),np.array(des2,dtype=np.float32), k=2)
                 ...
                 for m, n in matches:
                     if m.distance < 0.7 * n.distance:
                         good.append(m)
                 results.append((X_name[index], len(good)))
